...
$().ready(function() {
	...
	var $browserButtonImage = $("#browserButtonImage");
	
	[@flash_message /]
	
	$browserButtonImage.browser();
	...
	$inputForm.validate({
		...
		messages: {
			email: {
				pattern: "${message("admin.validate.email")}"
			},
			caption: {
				pattern: "${message("admin.article.captionMaxLength")}"
			}
		}
	});
});
...
<body>
	...
	<form id="inputForm" action="update.jhtml" method="post">
		...
		<table class="input">
			...
			<tr>
				<th>
					${message("Article.email")}:
				</th>
				<td>
					<input type="text" name="email" class="text" value="${article.email}" maxlength="200" />
				</td>
			</tr>
			<tr>
				<th>
			...
			<tr>
				<th>
					${message("Article.links")}:
				</th>
				<td>
					<input type="text" name="links" class="text" value="${article.seoDescription}" maxlength="200" />
				</td>
			</tr>
			<tr>
				<th>
				...
				<td>
					<span class="fieldSet">
						<input type="text" id="image" name="image" class="text" value="${article.image}" maxlength="200" />
						<input type="button" id="browserButtonImage" class="button" value="${message("admin.browser.select")}" />
			...
			<tr>
				<th>
					${message("Article.caption")}:
				</th>
				<td>
					<input type="text" name="caption" class="text" value="${article.seoDescription}" maxlength="200" />
				</td>
			</tr>
			<tr>
				<th>
					${message("Article.focusFigureImage")}:
				</th>
				<td>
					<label>
						<input type="checkbox" name="focusFigure" value="true"[#if article.focusFigure] checked="checked"[/#if] />${message("Article.focusFigure")}
						<input type="hidden" name="_focusFigure" value="false" />
					</label>
				</td>
			</tr>
			<tr>
